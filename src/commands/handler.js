...
const { getSettings, updateSettings } = require('../store/groupSettings');
const warns = require('../store/warns');
const shadersStore = require('../store/shaders');
const metaStore = require('../store/meta');
...
const {
  ...
  buildShaderResult,
  buildShaderNotFound,
} = require('../features/mcpatch');
const { kickWithNotice } = require('../features/kickWithNotice');
const { getUserProfile, buildProfileCaption } = require('../features/profile');
...
const commands = {
  ...
  menu: {
    ...
    handler: async ({ sock, groupId }) => {
      await sock.sendMessage(groupId, buildMenuMessage());
    },
  },
  ping: {
    adminOnly: false,
  ...
  mcpatch: {
    ...
    handler: async ({ sock, groupId, args, isOwner: owner }) => {
      ...
      await sock.sendMessage(groupId, buildMcpatchInfo(version, link));
    },
  },
  shaders: {
    adminOnly: false,
    handler: async ({ sock, groupId, args }) => {
      ...
      if (!variant) {
        await sock.sendMessage(groupId, buildShadersIntro());
        return;
      }
      ...
      if (!version) {
        ...
        await sock.sendMessage(groupId, buildVersionPrompt(variant, filtered));
        return;
      }
      ...
      await sock.sendMessage(groupId, buildShaderResult(variant, version, link));
    },
  },
  user: {
    adminOnly: false,
